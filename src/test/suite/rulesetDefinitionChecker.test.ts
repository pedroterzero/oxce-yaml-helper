...
const findDiagnostic = (file: string, expected: string, line?: number, char?: number) => {
    ...
    if (line && char) {
        return diagnosticsFile.find(
            (item) =>
                item.message === expected && item.range.start.line === line && item.range.start.character === char,
        );
    }
...
describe('rulesetDefinitionChecker', () => {
    ...
    it('finds a diagnostic for non existing item', () => {
        const diagnostic = findDiagnostic(
            'items.rul',
            '"STR_DUMMY_AMMO" does not exist (items.compatibleAmmo[]) for STR_ARRAY_REFERENCE',
        );
    ...
    it('finds a diagnostic for non existing builtin-type', () => {
        const diagnostic = findDiagnostic(
            'globalVariables.rul',
            '"STR_DUMMY_STATUS" does not exist (startingBase.crafts[].status) for STR_DUMMY_CRAFT',
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('does not find a diagnostic for existing builtin-type (int)', () => {
        const diagnostic = findDiagnostic(
            'items.rul',
            '"3" does not exist (items.meleeAnimation) for STR_BUILTIN_TYPE_TEST',
        );
    ...
    it('does not find a diagnostic for existing builtin-type (string)', () => {
        const diagnostic = findDiagnostic(
            'units.rul',
            '"STR_LIVE_COMMANDER" does not exist (units.rank) for STR_DUMMY_UNIT2',
        );
        assert.strictEqual(diagnostic, undefined);
    });
    ...
    it('does not find a diagnostic for existing builtin-type regex', () => {
        const diagnostic = findDiagnostic(
            'globalVariables.rul',
            '"STR_READY" does not exist (startingBase.crafts[].status) for STR_DUMMY_CRAFT',
        );
        assert.strictEqual(diagnostic, undefined);
    });
    ...
    it('does not find a diagnostic for string type', () => {
        const diagnostic = findDiagnostic(
            'items.rul',
            '"STR_DUMMY_STRING_TYPE" does not exist (items.name) for STR_STRING_TYPE_TEST',
        );
        assert.strictEqual(diagnostic, undefined);
    });

    it('finds a diagnostic for duplicate type', async () => {
        // just to be sure, enable it (less flaky tests)
        await workspace.getConfiguration('oxcYamlHelper').update('findDuplicateDefinitions', true);
        ...
        const diagnostic = findDiagnostic(
            'items.rul',
            'items STR_DUPLICATE_CHECK is duplicate (add # ignoreDuplicate after this to ignore this entry)',
        );
        assert.notStrictEqual(diagnostic, undefined);
    });

    it('does not find a diagnostic for ignored duplicate type', () => {
        const diagnostic = findDiagnostic(
            'items.rul',
            'items STR_DUPLICATE_IGNORE_CHECK is duplicate, also exists in (add # ignoreDuplicate after this to ignore this entry):\n\titems.rul line 48',
        );
        assert.strictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for an invalid category', async () => {
        // await workspace.getConfiguration('oxcYamlHelper').update('validateCategories', 'yes');
        // await waitForRefresh(rulesetResolver);
    ...
    it('finds a diagnostic for an invalid globalVariables reference', () => {
        const diagnostic = findDiagnostic(
            'globalVariables.rul',
            '"STR_DUMMY_NON_EXISTING_RESEARCH" does not exist (globalVariables.newBaseUnlockResearch)',
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for an incorrect missionZone', () => {
        const diagnostic = findDiagnostic(
            'regions.rul',
            'Crossing the prime meridian requires a different syntax (change to [2,361,3,-4] to fix this). See wiki.',
        );
        assert.notStrictEqual(diagnostic, undefined);
    });

    const ufopaediaImageErrorMessage =
        'Ufopaedia articles with type_ids 1, 2, 7, 10, 11, 12, 13, 14, 15, 16, 17 must have an image_id. Otherwise this will cause a segmentation fault when opening the article!';
    ...
    it('finds a diagnostic for an ufopaedia without type_id', () => {
        const diagnostic = findDiagnostic(
            'ufopaedia.rul',
            `'STR_ARTICLE_WITHOUT_TYPE_ID' does not have a type_id: set. Without it, the article will not appear in-game.`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for a craftweapon with an incorrect rearmRate', () => {
        const diagnostic = findDiagnostic(
            'craftWeapons.rul',
            "rearmRate of '10' is less than clipSize '25' for clip 'STR_REARM_RATE_TEST_AMMO'. This will cause a crash on loading OpenXcom!",
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('does not finds a diagnostic for a craftweapon with a correct rearmRate', () => {
        const diagnostic = findDiagnostic(
            'craftWeapons.rul',
            "rearmRate of '50' is less than clipSize '25' for clip 'STR_REARM_RATE_TEST_AMMO'. This will cause a crash on loading OpenXcom!",
            10,
            15,
        );
        assert.strictEqual(diagnostic, undefined);
    ...
    it('does not find a diagnostic for a mapscript command with correct groups', () => {
        let diagnostic = findDiagnostic(
            'mapScripts.rul',
            `'Group '2' does not exist in terrain for TEST_GOOD_MAPSCRIPT. This will cause a segmentation fault when loading the map!`,
            13,
            21,
        );
        assert.strictEqual(diagnostic, undefined);
    ...
    it('finds a diagnostic for a mapscript command (by terrain:) with incorrect groups', () => {
        const diagnostic = findDiagnostic(
            'mapScripts.rul',
            `'Group '666' does not exist in terrain for TEST_GOOD_MAPSCRIPT_BY_TERRAIN. This will cause a segmentation fault when loading the map!`,
            21,
            21,
        );
        assert.notStrictEqual(diagnostic, undefined);
    ...
    it('does not find a diagnostic for a mapscript command (by terrain:) with correct groups', () => {
        const diagnostic = findDiagnostic(
            'mapScripts.rul',
            `'Group '9' does not exist in terrain for TEST_GOOD_MAPSCRIPT_BY_TERRAIN. This will cause a segmentation fault when loading the map!`,
            28,
            21,
        );
        assert.strictEqual(diagnostic, undefined);
    ...
    it('finds a diagnostic for an item with a tu cost but no accuracy', () => {
        ...
        diagnostic = findDiagnostic(
            'items.rul',
            `if there's a TU cost for Snap, there should be an accuracy setting!`,
            94,
            12,
        );
        assert.notStrictEqual(diagnostic, undefined);
        diagnostic = findDiagnostic(
            'items.rul',
            `if there's a TU cost for Auto, there should be an accuracy setting!`,
            95,
            12,
        );
        assert.notStrictEqual(diagnostic, undefined);
        diagnostic = findDiagnostic(
            'items.rul',
            `if there's a TU cost for Melee, there should be an accuracy setting!`,
            96,
            13,
        );
        assert.notStrictEqual(diagnostic, undefined);
    ...
    it('finds a diagnostic for a craftweapon without a launcher', () => {
        const diagnostic = findDiagnostic(
            'craftWeapons.rul',
            `'STR_NO_LAUNCHER_TEST' does not have a launcher: set. This will cause a crash on loading OpenXcom!`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for a craftweapon with a launcher that is not an item', () => {
        const diagnostic = findDiagnostic(
            'craftWeapons.rul',
            `'STR_LAUNCHER_WRONG_TYPE_TEST' launcher 'STR_DUMMY_MANUFACTURE': item does not exist. This will cause a crash on loading OpenXcom!`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for a soldierTransformation without allowedSoldierTypes', () => {
        const diagnostic = findDiagnostic(
            ...
            `'STR_TRANSFORMATION_WITHOUT_SOLDIER_TYPES' does not have allowedSoldierTypes: set. Without it, it can never be used in-game.`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for alienDeployments without data', () => {
        const diagnostic = findDiagnostic(
            ...
            `'STR_TEST_ALIEN_DEPLOYMENT_NO_DATA' does not have data: set. This can lead to crashes in-game.`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds diagnostics for an alienDeployments with missing data', () => {
        ...
        for (const entry of expected) {
            for (const key of entry.missing) {
                const diagnostic = findDiagnostic(
                    'alienDeployments.rul',
                    `data entry '${key}' not set. This can lead to crashes in-game.`,
                    entry.line,
                    entry.char,
                );
                assert.notStrictEqual(diagnostic, undefined);
    ...
    it('finds a diagnostic for a manufactureShortcut without startFrom', () => {
        const diagnostic = findDiagnostic(
            ...
            `'STR_MANUFACTURE_SHORTCUT_WITHOUT_STARTFROM' does not have startFrom: set. This will cause a segmentation fault on loading OpenXcom!`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for an alienMission without waves', () => {
        const diagnostic = findDiagnostic(
            'alienMissions.rul',
            `'STR_ALIEN_MISSION_WITHOUT_WAVES' does not have waves: set. This will lead to a segmentation fault when this mission triggers.`,
        );
        assert.notStrictEqual(diagnostic, undefined);
    });
    ...
    it('finds a diagnostic for an alienMission without raceWeights', () => {
        const diagnostic = findDiagnostic(
            'alienMissions.rul',
            `'STR_ALIEN_MISSION_WITHOUT_RACEWEIGHTS' does not have raceWeights: set here or in missionScripts. This will lead to a crash when this mission triggers.`,
            3,
            10,
        );
        assert.notStrictEqual(diagnostic, undefined);
    ...
    it('finds a diagnostic for an alienMission without raceWeights in missionScripts', () => {
        const diagnostic = findDiagnostic(
            'alienMissions.rul',
            `'STR_ALIEN_MISSION_IN_MISSIONSCRIPT_NO_RACEWEIGHTS' does not have raceWeights: set here or in missionScripts. This will lead to a crash when this mission triggers.`,
            14,
            10,
        );
        assert.notStrictEqual(diagnostic, undefined);
    ...
    it('does not find a diagnostic for existing builtin-type from linker.yml (string)', () => {
        const diagnostic = findDiagnostic(
            'items.rul',
            '"STR_BUILTIN_TEST" does not exist (items.builtInTest) for STR_LINKER_YML_BUILTIN_TEST_GOOD',
        );
        assert.strictEqual(diagnostic, undefined);
    });
